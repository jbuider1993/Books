...
getData(Index) ->
  gen_server:call(?MODULE, {getData, Index}, infinity).
...
handle_call({getData, Index}, _From, State) ->
  [{_Idx,  _Data}] = ets:lookup(?MEMSTORAGE, Index),  
  printOutRecord(_Data),
  {reply, size(_Data), State}.
...
addRealRecord(N) when N == (?MAXINDEX - 1) ->
   ok;
